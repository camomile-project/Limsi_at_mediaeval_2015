...
We first detail the fusion system baseline provided to all participants (we are both organizer and participant), then we describe the LIMSI system. These two fusion methods used the metadata provided to all participants. Finally we present the results obtain by these two approaches.


\section{Baseline}

After the written names extraction, face detection and tracking, and speech turns segmentation, we perform a speaker diarization based on the BIC criterion. 

\begin{figure}[htb]
 \centering
 \includegraphics[width=1.0\linewidth]{figs/baseline.pdf}
 \caption {Baseline fusion system overview}
 \label{fig:gene}
\end{figure}

Afterwards, we name speakers with the "Direct Speech Turn Tagging" method describe in~\cite{POIGNANT--INTERSPEECH--2012}. We, first, tagged speech turns with exactly one co-occurring written name, then on the remaining unnamed speech turns we find the one-to-one mapping that maximizes the co-occurrence duration between speaker clusters and written names (see~\cite{POIGNANT--INTERSPEECH--2012} for more details). Finally, we propagate the speaker identities on the co-occurring face tracks based on the speech turns/face tracks mapping.

\section{Naming multi-modal clusters}
...
Two deadlines were proposed for this evaluation. For the first one (July 1st) we tuned the parameters $\alpha$ and $\beta$ and the stopping criterion of the clustering process on the out-domain development set. For the second deadline (July 8th), we tuned these parameters with the evaluation proposed via the leaderboard (computed every six hours on a subset of the test set). We can see only a little improvement between them, showing that our method generalizes well.
